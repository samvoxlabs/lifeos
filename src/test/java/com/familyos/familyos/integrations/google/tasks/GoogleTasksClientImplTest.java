...
class GoogleTasksClientImplTest {
    ...
    void fetchTasksResolvesTaskListBeforeFetchingTasks() {
        ...
        server.expect(requestTo("https://tasks.googleapis.com/tasks/v1/lists/tasklist-1/tasks?maxResults=10&showCompleted=true&showHidden=false"))
                .andExpect(header("Authorization", "Bearer access-token"))
                .andRespond(withSuccess("""
    ...
    @Test
    void fetchTasksReturnsEmptyListWhenTaskItemsAreMissing() {
        RestClient.Builder builder = RestClient.builder();
        MockRestServiceServer server = MockRestServiceServer.bindTo(builder).build();
